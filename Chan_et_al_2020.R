...
if(!require("RCurl")) install.packages('RCurl', repos='http://cran.us.r-project.org') ### readr 1.3.1

urlfile1 <- getURL("https://raw.githubusercontent.com/CSHoggard/-use_wear_test/master/dataset.csv")
urlfile2 <- getURL("https://raw.githubusercontent.com/CSHoggard/-use_wear_test/master/comp_dataset_1.csv")
urlfile3 <- getURL("https://raw.githubusercontent.com/CSHoggard/-use_wear_test/master/comp_dataset_2.csv")

dataset <- read.csv(text = urlfile1)
...
fig1c <- ggplot(df3, aes(classification_id3, weight)) + geom_jitter(width = 0.15, size = 2, aes(colour = classification_id3)) + ylim(0,1) + labs(x = "Classification", y = "Weight (g)") + scale_colour_manual(values=c("#56B4E9", "#E69F00")) + theme_minimal() + theme(legend.position = "none")
...
glm1 <- glm(classification_id1 ~ weight, df1, family = binomial) ### generalised linear model (binomial)
glm2 <- glm(classification_id2 ~ weight, df2, family = binomial) ### generalised linear model (binomial)
glm3 <- glm(classification_id3 ~ weight, df3, family = binomial) ### generalised linear model (binomial)
...
fig2a <- ggplot(df1, aes(x=weight, y=as.numeric(classification_id1) - 1)) + geom_point(size = 2, aes(colour = classification_id1)) + stat_smooth(method="glm", se=TRUE, colour = "dark grey", method.args = list(family=binomial)) +  labs(x = "Weight (g)", y = "Classification") + scale_x_continuous(breaks=c(0,0.25,0.5,0.75)) + scale_y_continuous(breaks=c(0,1)) + theme_minimal() + theme(legend.position = "none") + scale_colour_manual(values=c("#56B4E9", "#E69F00"))
fig2b <- ggplot(df2, aes(x=weight, y=as.numeric(classification_id2) - 1)) + geom_point(size = 2, aes(colour = classification_id2)) + stat_smooth(method="glm", se=TRUE, colour = "dark grey", method.args = list(family=binomial)) +  labs(x = "Weight (g)", y = "Classification") + scale_x_continuous(breaks=c(0,0.25,0.5,0.75)) + scale_y_continuous(breaks=c(0,1)) + theme_minimal() + theme(legend.position = "none") + scale_colour_manual(values=c("#56B4E9", "#E69F00"))
fig2c <- ggplot(df3, aes(x=weight, y=as.numeric(classification_id3) - 1)) + geom_point(size = 2, aes(colour = classification_id3)) + stat_smooth(method="glm", se=TRUE, colour = "dark grey", method.args = list(family=binomial)) +  labs(x = "Weight (g)", y = "Classification") + scale_x_continuous(breaks=c(0,0.25,0.5,0.75)) + scale_y_continuous(breaks=c(0,1)) + theme_minimal() + theme(legend.position = "none") + scale_colour_manual(values=c("#56B4E9", "#E69F00"))
